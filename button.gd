...
func _process(_delta: float) -> void:
	...
	wait_time = unchangedwaittime + (diceamount - Engine.get_meta("prestigecooldown", 0.01)) * 0.5
...
func _button_pressed() -> void:
	...
	for i in numb:
		if i == int(6):
			...
			if lis.size() >= 7:	
				uptree7mult *= 10
	var rollexponent = Engine.get_meta("rollexponent", 1)
	Engine.set_meta("currentcombo", combolist)
	Engine.set_meta("numb", numb)
	upgradeboostsmult = uptree7mult * dpboostmult
	score += int(pow(listmultiply(numb) * combomult * totalrollmult * upgradeboostsmult, rollexponent)) * Engine.get_meta("prestigemult", 1)
	scoregain = int(pow(listmultiply(numb) * combomult * totalrollmult * upgradeboostsmult, rollexponent)) * Engine.get_meta("prestigemult", 1)
	Engine.set_meta("scoregain", scoregain)
